...
function calculateEmotionalIndex() {
  const mood = Number(localStorage.getItem("mood"));
  const energy = Number(localStorage.getItem("energy"));
  const questionScore = Number(localStorage.getItem("questionScore"));

  const storedSentiment = localStorage.getItem("sentimentScore");
  const sentimentScore =
    storedSentiment !== null ? Number(storedSentiment) : 0.5;
  ...
  const finalEI = Math.max(0, Math.min(1, EI));

  localStorage.setItem("emotionalIndex", finalEI);
...
async function sendToBackend() {
  ...
  const body = {
    // Parse strings into Numbers so MongoDB accepts them correctly
    mood: parseInt(localStorage.getItem("mood")),
    energy: parseInt(localStorage.getItem("energy")),
    questions: parseInt(localStorage.getItem("questionScore")),
    journalEntry: localStorage.getItem("journalText"),
    score: parseFloat(localStorage.getItem("emotionalIndex")),
    sentimentScore: parseFloat(localStorage.getItem("sentimentScore"))
  };
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error("❌ Server rejected entry:", data.message);
    }
  } catch (err) {
...
"⚠️ Avoid isolation — seek human connection and professional support."];
  ...
  if (meter) {
    meter.style.width = `${(EI * 100).toFixed(1)}%`;
    meter.style.background = color;
  }
  document.getElementById("emotionalLevel").textContent = level;
  document.getElementById("suggestions").innerHTML = suggestion.map(item => `<div class="suggestion-item">${item}</div>`).join("");
  document.getElementById("description").textContent = description;
  if (!sessionStorage.getItem("assessmentSubmitted")) {
